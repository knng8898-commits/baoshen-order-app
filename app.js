...
function renderStores() {
  ...
  function renderItems() {
    ...
    APP_DATA.vendors[state.vendor].groups.forEach(group => {
      const title = document.createElement("div"); title.className = "group-title"; title.textContent = group.title; box.appendChild(title); group.items.forEach(item => {
        const saved = Number(
      ...
      vendor.groups.forEach((group) => {
        const lines = group.items
          .map((item) => {
            ...
            return `${item.name} ${quantity}${item.unit}${item.note ? ` ${item.note}` : ""
              }`;
          })
          .filter(Boolean);

...
function formatQuantityWithUnit(item,quantity){return `${formatQuantity(item,quantity)}${itemRule(item).quantityIncludesUnit?"":item.unit}`}
...
function renderItems(){
  $("inventoryMode").classList.toggle("active",state.mode==="inventory");$("orderMode").classList.toggle("active",state.mode==="order");const hasImage=Boolean(APP_DATA.vendors[state.vendor].image);$("generateImage").style.display=hasImage?"inline-block":"none";$("quickGenerateImage").classList.toggle("hidden",!hasImage);
  ...
  APP_DATA.vendors[state.vendor].groups.forEach(group=>{const title=document.createElement("div");title.className="group-title";title.textContent=group.title;box.appendChild(title);group.items.forEach(item=>{
    const saved=normalizeQuantity(item,localStorage.getItem(quantityKey(item.name))||0),target=suggested(item);const row=document.createElement("div");row.className="item";
    const info=document.createElement("div");info.innerHTML=`<div class="item-name">${escapeHtml(item.name)}</div><div class="item-meta">單位：${escapeHtml(item.unit)}${item.note?"・"+escapeHtml(item.note):""}</div>${item.weekday!==undefined||item.holiday!==undefined?`<div class="item-suggested">建議庫存：${escapeHtml(formatQuantityWithUnit(item,target))}</div>`:""}${state.mode==="inventory"&&saved===0?'<div class="item-warning">請確認：目前庫存為 0</div>':""}`;
    const step=document.createElement("div");step.className="stepper";const minus=document.createElement("button"),input=document.createElement("input"),plus=document.createElement("button"),increment=itemStep(item),hasCustomStep=Boolean(itemRule(item).step);minus.textContent="−";plus.textContent="+";input.type=hasCustomStep?"text":"number";input.min="0";input.className="quantity";input.dataset.itemName=item.name;input.dataset.quantity=saved;input.value=formatQuantity(item,saved);input.readOnly=hasCustomStep;
    const update=delta=>{const value=Math.max(0,(Number(input.dataset.quantity)||0)+delta);input.dataset.quantity=value;input.value=formatQuantity(item,value);localStorage.setItem(quantityKey(item.name),value);renderItems()};minus.onclick=()=>update(-increment);plus.onclick=()=>update(increment);if(!hasCustomStep){input.oninput=()=>{input.dataset.quantity=String(Math.max(0,Number(input.value)||0));localStorage.setItem(quantityKey(item.name),input.dataset.quantity)};input.onchange=()=>{input.dataset.quantity=String(Math.max(0,Number(input.value)||0));input.value=input.dataset.quantity;localStorage.setItem(quantityKey(item.name),input.dataset.quantity);renderItems()}}step.append(minus,input,plus);
    if(state.mode==="inventory"){const r=document.createElement("div");r.className="order-result";r.textContent=`叫 ${formatQuantityWithUnit(item,Math.max(0,target-saved))}`;step.appendChild(r)}row.append(info,step);box.appendChild(row)
  })})
}
...
function generateMessage(){
  ...
  }else vendor.groups.forEach(group=>{const lines=group.items.map(i=>{const q=orderQuantity(i);return q>0?`${vendorItemName(i)} ${formatQuantityWithUnit(i,q)}${i.note?" "+i.note:""}`:null}).filter(Boolean);if(lines.length)sections.push((group.outputTitle?group.outputTitle+"\n":"")+lines.join("\n"))});
